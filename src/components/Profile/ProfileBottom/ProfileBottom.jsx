  ...
  return (
    <>
      ...
      >
        ...
        >{status}</span>
        <div className={`statusSale ${status === 'Продан' || status === 'Вывод в процессе' ? 'sold' : ''}`}  onClick={status === 'Продан' || status === 'Вывод в процессе' ? null : () => handleSaleItem(id)}>{price} ₽ {status !== 'Продан' && <span>Продать</span>}</div>
        <div className='withdraw' style={ status === 'Продан' || status === 'Вывод в процессе' ? {display: 'none'} : {display: 'flex'} } onClick={() => handleWithdrawalItem(id)}>
